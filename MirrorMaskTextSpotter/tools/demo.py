...
def main(args):
    ...
    image_path = [f for f in listdir(args.image_path) if isfile(join(args.image_path, f))]

    # print(image_path)
    for imageX in image_path:
        print(imageX)
        start = time.time()
        image = cv2.imread(os.path.join(args.image_path,imageX))
        ...
        resultText = 'result'+imageX
        cv2.imwrite(os.path.join(args.visu_path,resultText), image)
        end = time.time()
        print(f'RT :  {end - start}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_path", type=str, default='./demo_images/')
    parser.add_argument("--visu_path", type=str, default='./demo_images/demo_results/')
    args = parser.parse_args()
    startX = time.time()
    main(args)
    endX = time.time()
    print(f"Total time taken: {endX - startX}")
